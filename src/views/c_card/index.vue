  ...
  <div class="app-container">
    ...
    <div class="card_table_container">
      <el-table
        ...
        fit
        sortable
        element-loading-text="正在幫你載入所有卡片資料中......."
    ...
    <div class="dialog_container">
      <el-dialog
        :visible.sync="c_card_visible"
        :title="$t('c_card_view.edit')"
        width="80vw"
      >
        <el-form
          ref="c_card_edit"
          :model="c_card_edit"
          :rules="c_card_edit_rules"
        ...
        >
          ...
          >
            <el-input
              v-model="c_card_edit.name"
              :placeholder="c_card_name_p"
              clearable
              name="name"
              @focus="clean_name()"
            />
          ...
          >
            <el-input
              v-model="c_card_edit.number"
              :placeholder="c_card_number_p"
              clearable
              name="number"
              @focus="clean_number()"
            />
        ...
        <span
          slot="footer"
          class="card_dialog_footer"
        >
          <el-button
            type="danger"
            plain
            @click.native.prevent="c_card_del()"
          >{{ $t('c_card_view.delete') }}</el-button>
          <el-button
            type="primary"
            @click.native.prevent="c_card_confirm()"
          >{{ $t('c_card_view.confirm') }}</el-button>
          <el-button
            type="info"
            plain
            @click.native.prevent="c_card_cal()"
          >{{ $t('c_card_view.cancel') }}</el-button>
        </span>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      c_card_edit_rules: {
        name: [
          { required: true, trigger: 'change', validator: validecardnamelength }
        ],
        number: [
          {
            required: true,
            trigger: 'change',
            validator: validecardnumberlength
          }
        ]
      },
      view_loading: true
  ...
  methods: {
    ...
    get_card() {
      getcard(getToken(), this.listQuery)
        .then(response => {
          ...
          this.$confirm('你真的要刪除該卡片資料嗎？', '警告', {
            ...
            .then(() => {
              this.$confirm('請在確認一次是否要刪除該卡片資料資料', '警告', {
                ...
                .then(() => {
                  const date = formatdate('yyyy-mm-dd HH:MM:ss.l')
                  patchcard_del(getToken(), this.c_card_id, date)
                    .then(() => {
                      this.$message({
                        type: 'success',
                        message: '刪除成功'
