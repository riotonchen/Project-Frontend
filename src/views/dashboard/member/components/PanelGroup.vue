      ...
      <el-col :xs="24" :sm="12" :lg="6" class="card-panel-col">
        <div class="card-panel">
          ...
          <div class="card-panel-description">
            ...
            <count-to :start-val="0" :end-val="c_category_m_list" :duration="3600" class="card-panel-num" />
          </div>
        </div>
      </el-col>
    </el-row>
        ...
        .then((res_month) => {
          getaccounting_all(getToken()
            ...
            .then((res_year) => {
              ...
              this.c_user_y_history_pay = this.c_user_y_history.filter(function(item) {
                return item.type === '支出'
              })

              this.c_user_y_history_pay = this.c_user_y_history_pay.reduce(function(accumulator, currentValue) {
                return accumulator + currentValue.amount
              }, 0)
              this.c_user_y_history_in = this.c_user_y_history.filter(function(item) {
                return item.type === '收入'
              })
              this.c_user_y_history_in = this.c_user_y_history_in.reduce(function(accumulator, currentValue) {
                return accumulator + currentValue.amount
              }, 0)
              this.c_user_y_history_balance = this.c_user_y_history_in - this.c_user_y_history_pay
              ...
              this.c_user_m_history_pay = this.c_user_m_history.filter(function(item) {
                return item.type === '支出'
              })

              this.c_user_m_history_pay = this.c_user_m_history_pay.reduce(function(accumulator, currentValue) {
                return accumulator + currentValue.amount
              }, 0)
              this.c_user_m_history_in = this.c_user_m_history.filter(function(item) {
                return item.type === '收入'
              })
              this.c_user_m_history_in = this.c_user_m_history_in.reduce(function(accumulator, currentValue) {
                return accumulator + currentValue.amount
              }, 0)
            })
        })
    ...
    get_getproject_all() {
      ...
      getsortbudget_all(getToken()
        ...
        .then((res_month) => {
          ...
          this.c_category_m_list = this.c_category_m_list.reduce(function(accumulator, currentValue) {
            return accumulator + currentValue.balance
          }, 0)
        })
    }
