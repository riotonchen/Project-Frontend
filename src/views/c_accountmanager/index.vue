<template>
  <div class="app-container">
    ...
    <div class="history_table_container">
      <el-table :data="c_user_account" stripe style="width: 100%;min-width:20rem;max-width:33rem;" max-height="470" fit sortable>
        <el-table-column prop="id" label="編號" width="70">
          <template slot-scope="scope">
            <span>{{ scope.row.id }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="name" label="帳戶名稱" width="150">
          <template slot-scope="scope">
            <span>{{ scope.row.name }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="number" label="餘額" width="200">
          <template slot-scope="scope">
