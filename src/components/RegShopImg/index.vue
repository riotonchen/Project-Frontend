<template>
  ...
  >
    <div
      ...
      class="upload_btn"
    >
      <div style="height:40px" /><i class="iconfont icon-jiahao" />
    ...
    changeUpload(file, fileList) {
      ...
      if (fileList[0].size < 1100000) {
        ...
        if (
          ...
          this.$nextTick(() => {
            var i = this.imgN
            const uploadLists = document.getElementsByClassName(
              'el-upload-list'
            )
            const uploadListsN = uploadLists[i]
            const uploadListLi = uploadListsN.children
            uploadListsN.setAttribute(
              'style',
              'position: absolute;height: 160px;margin-top: 0;margin-left: 300px;width: 260px;overflow: hidden'
            )
            const liA = uploadListLi[0]
            // 试着获取bolb里面的数据------------S
            // 获取图片的Blob值
